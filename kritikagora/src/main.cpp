...
int commitCheck(std::string checkpath)
{
	...
	for(auto &p : fs::recursive_directory_iterator(commitFolderPath))
	{
		...
		if(stat(p.path().c_str(), &buffer) == 0 && stat((root + filename).c_str(), &buffer2) == 0)
		{
			
			if(buffer.st_mode & S_IFDIR && buffer2.st_mode & S_IFDIR)
				continue;
			else if(buffer.st_mode & S_IFREG && buffer2.st_mode & S_IFREG)
			{
...
void revert(char **argv)
{
	...
	else if(dirBool && lastCommit != "")
	{
		if(commitCheck(lastCommit) == 0)
		{
				...
				if(lastCommit == passedHash)
				{
					...
					for(auto &p : fs::recursive_directory_iterator(root))
					{
						...
						if(stat(p.path().c_str(), &buffer) == 0)
						{
							...
							{
								if(stat(prevpath.c_str(), &buffer2) != 0 && stat(revertpath.c_str(), &buffer3) == 0)
									remove(p.path());
							}
						}
					}
					...
					for(auto &p : fs::recursive_directory_iterator(commitFolderPath))
					{
						...
						if(stat(p.path().c_str(), &buffer) == 0)
						{
							...
							else if(buffer.st_mode & S_IFREG)
							{
								...
								if(stat(rootPath.c_str(), &buffer2) == 0)
								{
					...
					for(auto &p : fs::recursive_directory_iterator(root + "/.imperium/.commit/" + passedHash))
					{
						...
						if(stat(p.path().c_str(), &buffer) == 0)
						{
							...
							else if(buffer.st_mode & S_IFREG)
							{
								...
								std::string rootPath = root + relpath;
								std::string prevpath = commitFolderPath + relpath;
								if(stat(rootPath.c_str(), &buffer2) == 0 && stat(prevpath.c_str(), &buffer3) != 0)
								{
									...
									else
									{
										...
										std::cout<<"\033[1;31mMERGE CONFLICT IN: \033[0m\n"<<rootPath<<"\n";
									}
								}
							}
						}
			...
			std::cout<<"\033[1;31mYou have changes that need to be commited or stashed.\033[0m\n";
	}
	else
	{
		std::cout<<"No commit to revert\n";
		return;
	}
}
...
int main(int argc, char **argv)
{
	...
	else if(conflictCheck() == 0)
	{
		...
		std::cout<<"\033[1;31mYou have to make a commit, after resolving any conflicts.If there, type 'imperium resolve'\033[0m\n";
	return 0;
}